...
def main(model, results_dir):
    ...
    for ch_idx in np.arange(n_channels):
        ...
        for item_idx, item in enumerate(max_active_nodes):
            ...
            # Find the closet point on the contour
            d_to_contour = data_set.get_distance_point_and_contour(max_act_idx, item.c1 // 4)
            closest_contour_point_idx = np.argmin(d_to_contour)
            closest_contour_point = item.c1[closest_contour_point_idx]

            relative_colinear_distance = 1
            new_img1 = data_set.get_img_by_index(item.index, item.ep1, item.ep2)
            puncture = utils.PunctureImage(n_bubbles=1, fwhm=7, tile_size=[14, 14])

            bubble_center = np.array(closest_contour_point - np.array([14,14]) // 2)


            punctured_img = puncture(new_img1, start_loc_arr=bubble_center)
            plt.figure()
            ...
            # Process the image
            new_img = torch.unsqueeze(punctured_img, dim=0)
            label_out = process_image(model, dev, ch_mean, ch_std, new_img)
            ...
            ax_arr[1].scatter(
                closest_contour_point[1] // 4, closest_contour_point[0] // 4,
                marker='+', color='green', s=120)
