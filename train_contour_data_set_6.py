...
# positive lateral weights.
#
# Currently this is enforced by adding a loss term for negative
# weights (difference from train_contour_data_set_5.py)
#
...
def main(model, train_params, data_set_params, base_results_store_dir='./results'):
    ...
    def train():
        ...
        for iteration, (img, label) in enumerate(train_data_loader, 1):
            ...
            neg_w_loss = negative_weights_loss(
                model.contour_integration_layer.lateral_e.weight,
                model.contour_integration_layer.lateral_i.weight
            )

            total_loss = batch_loss + lambda1 * kernel_loss + lambda2 * neg_w_loss
