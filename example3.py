...
def main(model, results_dir):
    ...
    for ch_idx in np.arange(n_channels):
        ...
        puncture = utils.PunctureImage(n_bubbles=1, fwhm=frag_tile_size[0], tile_size=full_tile_size)
        ...
        for item_idx, item in enumerate(max_active_nodes):
            ...
            new_img = np.ones((3, 256, 256))

            lhs_bubbles_idx = np.arange(closest_contour_point_idx+7, len(item.c1), full_tile_size[0])
            rhs_bubbles_idx = np.arange(closest_contour_point_idx-7, 0, -full_tile_size[0])
            ...
            bubble_insert_locations = bubble_insert_locations - full_tile_size // 2

            bubble_insert_locations = np.array([256 // 2, 256 // 2] - full_tile_size // 2 + np.array((1, 1)))
            ...
            ax_arr[1].scatter(closest_contour_point[1], closest_contour_point[0], marker='+', color='red', s=120)
            ax_arr[1].scatter(item.c1[:, 1], item.c1[:, 0], marker='.', color='magenta')

            #
            # new_img = data_set.get_img_by_index(item.index, item.ep1, item.ep2)
            # punctured_img = puncture(new_img, start_loc_arr=bubble_center)
            # # Process the image
