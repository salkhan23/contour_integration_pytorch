# ------------------------------------------------------------------------------------
# Similar to original script but with L1 weight regularization
# ------------------------------------------------------------------------------------
...
def main(model, train_params, data_set_params, base_results_store_dir='./results'):
    ...
    lambda1 = train_params['reg_loss_weight']
    ...
    def train():
        ...
        for iteration, (img, label) in enumerate(train_data_loader, 1):
            ...
            kernel_loss = \
                model.contour_integration_layer.lateral_e.weight.norm(p=1) + \
                model.contour_integration_layer.lateral_i.weight.norm(p=1)
    ...
    file_handle.write("L1 loss weight        : {}\n".format(lambda1))
