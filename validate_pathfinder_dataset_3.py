...
#
# NOTE: This is similar to validate pathfinder_dataset_2.py(gets predictions from the
# individual contours and full label images) but for models that use ONLINE normalization.
# Each image is forced to have a 0 mean and 1 std
# ---------------------------------------------------------------------------------------
import numpy as np
...
if __name__ == "__main__":
    ...
    saved_model = \
        './results/pathfinder/' \
        'BinaryClassifierResnet50_CurrentSubtractInhibitLayer_20200704_142452__lr_1e-4_reg_1e-5_online_normalization/' \
        'best_accuracy.pth'
    ...
    with torch.no_grad():
        for iteration, data_loader_out in enumerate(data_loader, 1):
            ...
            transform_functional.normalize(
                torch.squeeze(individual_contours_labels), img_mean, img_std, inplace=True)
