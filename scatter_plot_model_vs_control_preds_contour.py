...
if __name__ == "__main__":
    ...
    gt_dir = './data/channel_wise_optimal_full14_frag7/val/labels'
    ...
    list_of_files = []
    for root, directories, filenames in os.walk(gt_dir):
        for filename in filenames:
            list_of_files.append(os.path.join(root, filename))

    model_list_of_files = []
    for root, directories, filenames in os.walk(model_predictions_dir):
        for filename in filenames:
            model_list_of_files.append(os.path.join(root, filename))

    control_list_of_files = []
    for root, directories, filenames in os.walk(control_predictions_dir):
        for filename in filenames:
            control_list_of_files.append(os.path.join(root, filename))
    ...
    model_list_of_files = sorted(model_list_of_files)
    control_list_of_files = sorted(control_list_of_files)
    ...
    for idx, img in enumerate(sorted(list_of_files)):
        print("[{}] processing file: {}".format(idx, img))
        ...
        gt = np.load(list_of_files[idx])
        model_out = np.load(model_list_of_files[idx])
        control_out = np.load(control_list_of_files[idx])

        # print(list_of_files[idx])
        # print(model_list_of_files[idx])
        # print(control_list_of_files[idx])
        #
        ...
        # Process Non-Edges
        # -------------------------------------------------------------------------------
        non_edges_mask = np.abs(gt.astype('int') - 1)
