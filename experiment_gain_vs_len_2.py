...
def find_optimal_stimulus(
    ...
    for base_gp_idx, base_gabor_params in enumerate(base_gabor_parameters):
        ...
        for o_idx, orient in enumerate(orient_arr):
            ...
            angle = g_params[0]["theta_deg"]
            d = 6 + frag_size[0] // 2
            loc = np.array([d * np.cos(angle * np.pi / 180.), d * np.sin(angle * np.pi / 180.)])

            add_end_stop(test_img, img_center + loc, radius=6)
            add_end_stop(test_img, img_center - loc, radius=6)
...
def get_contour_gain_vs_length(
    ...
    for c_len_idx, c_len in enumerate(c_len_arr):
        ...
        for img_idx in range(n_images):
            ...
            # Add End Stops
            d = 6 + frag_size[0] // 2
            angle = start_acc_angle + g_params[0]['theta_deg']
            x = np.int(d * (np.cos(angle * np.pi / 180.)))
            y = np.int(d * (np.sin(angle * np.pi / 180.)))
...
def add_end_stop(img1, loc=(0, 0), radius=8):
    """
    img1 should be 3d (channel first) and a pytorch variable
    """
    ax = torch.arange(loc[0] - radius, loc[0] + radius + 1)
    ay = torch.arange(loc[1] - radius, loc[1] + radius + 1)
        ...
        for x in ax:
            for y in ay:
                if ((0 <= x < img1.shape[1]) and (0 <= y < img1.shape[2])
                        and ((x - loc[0]) ** 2 + (y - loc[1]) ** 2) <= radius ** 2):
                    x = x.int()
                    y = y.int()
                    img1[:, x, y] = 0
...
if __name__ == "__main__":
    ...
    main(net, results_dir, iou_results=False)
