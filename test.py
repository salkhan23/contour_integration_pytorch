# ------------------------------------------------------------------------------------
#  Train a contour integration model on edge extractions from natural images
# ------------------------------------------------------------------------------------
...
def main(model, train_params, data_set_params, base_results_store_dir='./results'):
    ...
    train_transforms = transforms.Compose([
        normalize,
        utils.PunctureImage(n_bubbles=100, bubble_sigma=2.0, tile_size=[11, 11])
    ])

    train_set = dataset_edge.EdgeDataSet(
        data_dir=os.path.join(data_set_dir, 'train'), transform=train_transforms, subset_size=train_subset_size)
    ...
    val_set = dataset_edge.EdgeDataSet(
        data_dir=os.path.join(data_set_dir, 'val'), transform=train_transforms, subset_size=test_subset_size)
    ...
    def train():
        ...
        for iteration, (img, label) in enumerate(train_data_loader, 1):
            ...
            # ax_arr[0].imshow(np.transpose(img[img_idx, ].cpu().detach().numpy(), axes=(1, 2, 0)))
            # ax_arr[1].imshow(label[img_idx, ].squeeze().cpu().detach().numpy())
            # import pdb
            # pdb.set_trace()
...
if __name__ == '__main__':
    ...
    main(net, train_params=train_parameters, data_set_params=data_set_parameters,
         base_results_store_dir='./results/test')
